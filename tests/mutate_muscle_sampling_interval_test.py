...
new_muscle = muscle.mutate_sampling_interval()

x2 = [i for i in range(new_muscle.period)]
y2 = [new_muscle.get_activation(i) for i in range(new_muscle.period)]


plt.plot(x1, y1, label=f'muscle activation {muscle.sampling_interval}')
plt.plot(x2, y2, label=f'muscle activation {new_muscle.sampling_interval}')
