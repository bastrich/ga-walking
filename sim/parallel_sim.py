...
class ParallelSim:
    ...
    def run(self, walking_strategies, number_of_steps=1000):
        futures = [self.sims_executor.submit(self.simulate, walking_strategy, number_of_steps) for walking_strategy in walking_strategies]
        return [future.result() for future in futures]

    def simulate(self, walking_strategy, number_of_steps):
        sim = self.sims.get()
